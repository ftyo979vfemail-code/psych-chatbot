...
def chat(req: ChatRequest, db: Session = Depends(get_db)):
    ...
    bot_msg = models.Message(session_id=session.id, role="assistant", content=reply_text)
    db.add(bot_msg)
    db.flush()
    for s in sources:
        db.add(models.MessageSource(message_id=bot_msg.id, title=s.title, url=s.url))

    db.commit()

    return ChatResponse(reply=reply_text, sources=sources, session_id=session.id)


@app.get("/sessions", response_model=List[SessionOut])
def list_sessions(db: Session = Depends(get_db)):
    sessions = db.query(models.ChatSession).order_by(models.ChatSession.updated_at.desc()).all()
    return sessions
